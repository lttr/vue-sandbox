<template>
  <div>
    ...
    <section>
      ...
      with source
      <pre><code>{{ JSON.stringify(sourceForBar) }}</code></pre>
    ...
    <section>
      ...
      with source
      <pre><code>{{ JSON.stringify(sourceForBaz) }}</code></pre>
    ...
    <section>
      <div>
        <code>{{ reactiveArray }}</code>
      </div>
      <div>
        <code>{{ refArray }}</code>
      </div>
      <button @click="onChangeArray">Change something</button>
    </section>
  </div>
</template>
...
const refArray = ref(['d', 'e', 'f'])
const something = ref(false)
function onChangeArray() {
  something.value = !something.value
}
watchEffect(() => {
  const chars =
    String.fromCharCode(Math.floor(Math.random() * 26) + 97) + something.value.toString()[0]
  reactiveArray.unshift(chars)
  // refArray.value.unshift(chars)
  refArray.value = refArray.value.toSpliced(0, 0, chars)
})
</script>
